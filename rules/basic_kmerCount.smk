rule countKmer1:
    ...
    output:
        f"{dataDir}/{{sample}}/kmer{kmerSize}/intermediate/reverse{{seg}}.int.kmc_pre",
        f"{dataDir}/{{sample}}/kmer{kmerSize}/intermediate/reverse{{seg}}.int.kmc_suf"
    params:
        kmerSize = config["kmerSize"],
        memory = config["kmc3"]["memory"],
        maxCount = config["kmc3"]["maxCount"],
        thread = config["thread"],
        output = f"{dataDir}/{{sample}}/kmer{kmerSize}/intermediate/reverse{{seg}}.int",
