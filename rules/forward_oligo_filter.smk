rule forward_parameters:
    input:
        datadir + "/{sample}/" + kmerCounter + "/forward{seg}.kCountSorted"
    output:
        datadir + "/{sample}/filtering/forward{seg}.calculated"
    params:
...
rule linguistic_complexity:
    ...
    shell:
        "lib/nessie/nessie -I {input} -O {output} -L -k 1 -K 2"
...
rule add_LC:
    ...
    run:
        ...
        ### write to tsv format
        outputFile.write("position\tkmerCount\tCG%\tTm\thomodimer-dG\thairpin-dG\tLC\tkmer\n")
        for li2 in input2:
            li2 = li2.rstrip("\n")
            if li2.startswith(">"):
                ls = li2.split("|")
...
checkpoint filterOut_within_segments:
    input:
        datadir + "/{sample}/filtering/forward_{seg}.calculated2"
    output:
