rule countKmer1:
    input:
        f"{dataDir}/{{sample}}{clustering_identity}/splitFiles/reverse{{seg}}.fasta"
    output:
        f"{dataDir}/{{sample}}{clustering_identity}/kmer{kmerSize}/intermediate/reverse{{seg}}.jf"
    params:
...
rule countKmer2:
    input:
        f"{dataDir}/{{sample}}{clustering_identity}/kmer{kmerSize}/intermediate/reverse{{seg}}.jf"
    output:
        f"{dataDir}/{{sample}}{clustering_identity}/kmer{kmerSize}/intermediate/reverse{{seg}}.Kmercount.txt"
    shell:
