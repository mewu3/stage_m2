rule filtering1:
    ...
    run:
        ...
        def max_vocabularies(seq_length): # from: https://github.com/bioinform/somaticseq/blob/e6f5b1c6b98b324d418407154392778164215a65/somaticseq/utilities/linguistic_sequence_complexity.py
            # According to:
        ...
        def LC(sequence): # from: https://github.com/bioinform/somaticseq/blob/e6f5b1c6b98b324d418407154392778164215a65/somaticseq/utilities/linguistic_sequence_complexity.py
...
rule filtering2:
    ...
    run:
        ...
        # MSSPE Deng et al. (2020), don't know the logic behind yet. with
        # tm range from 60 - 70 no oligo could pass the criteria
        TmSeuilPlus = tmMean + 2*tmStd
        TmSeuilLess = tmMean - 2*tmStd

        df_filtered = df[(df["Tm"] >= TmSeuilLess) & (df["Tm"] <= TmSeuilPlus) & (df["CG%"] >= GCUp) & (df["CG%"] <= GCDown) & (df["hairpin-dG"] > deltaG) & (df["homodimer-dG"] > deltaG) & (df["Entropy"] >= entropyThreshold)]
