...
taxid = "/datater/wu/data/enterovirus/evaluation/taxid.txt"

oligoset_open = open(oligoSet, "r")
taxid = open(taxid, "w")

dictSeq = defaultdict(str)
dict = defaultdict(lambda: defaultdict(str))

for line in oligoset_open:
    line = line.rstrip("\n")
    if line.startswith(">"):
        header = line
        ...
        dictSeq[position] = oligo

for k in dictSeq:
    seq = dictSeq[k]
    splitFiles = f"{splitFilesDir}/reverse{k}.fasta"
    matchCount = 0
    accessionID_ls = []
    with open(splitFiles, "r") as file:
        for line in file:
            line = line.rstrip("\n")
            if line.startswith(">"):
                accessionID = line.split()[1]
                # taxID = os.popen(f"efetch -db nuccore -id {accessionID} -format docsum | xtract -pattern DocumentSummary -element TaxId").read().rstrip("\n")
                accessionID_ls.append(accessionID)
            if not line.startswith(">"):
                if re.search(seq, line, re.I):
                    matchCount += 1
    dict[k][matchCount]=accessionID_ls
    # print(k, matchCount)
    # print(accessionID_ls)

for position in dict :
    for matchCount in dict[k]:
        for id in dict[k][matchCount]:
            taxID = os.popen(f"efetch -db nuccore -id {id} -format docsum | xtract -pattern DocumentSummary -element TaxId").read()
            print(f"{position}\t{matchCount}\t{id}\t{taxID}")
